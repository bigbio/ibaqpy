...
def peptide_normalization(msstats: str, parquet: str, sdrf: str, min_aa: int, min_unique: int, remove_ids: str,
    ...
    if parquet is None:
        ...
        dataset_df[PROTEIN_NAME] = dataset_df.apply(lambda x: ",".join(x[PROTEIN_NAME]), axis = 1)
    ...
    dataset_df[PEPTIDE_CANONICAL] = dataset_df.apply(lambda x: get_canonical_peptide(x[PEPTIDE_SEQUENCE]), axis=1)
    # Only peptides with more than min_aa (default: 7) amino acids are retained
    dataset_df = dataset_df[dataset_df.apply(lambda x: len(x[PEPTIDE_CANONICAL]) >= min_aa, axis=1)]
    # Only proteins with unique peptides number greater than min_unique (default: 2) are retained
    unique_peptides = set(dataset_df.groupby(PEPTIDE_CANONICAL).filter(lambda x: len(set(x[PROTEIN_NAME])) == 1)[
                            PEPTIDE_CANONICAL].tolist())
    strong_proteins = set(dataset_df[dataset_df[PEPTIDE_CANONICAL].isin(unique_peptides)].groupby(PROTEIN_NAME).filter(
