...
def peptide_normalization(msstats: str, sdrf: str, remove_ids: str, remove_decoy_contaminants: bool, remove_low_frequency_peptides: bool, 
                          output: str, skip_normalization: bool, min_aa: int, min_unique: int, nmethod: str, compress: bool,
    ...
    for msstats_df in msstats_chunks:
        msstats_df.rename(
            columns={'ProteinName': PROTEIN_NAME, 'PeptideSequence': PEPTIDE_SEQUENCE,
        ...
        if PEPTIDE_CANONICAL not in msstats_df.columns:
            ...
            msstats_df[PEPTIDE_CANONICAL] = msstats_df.apply(lambda x: inner_canonical_dict[x[PEPTIDE_SEQUENCE]],
                                                             axis=1)
            ...
            if nmethod == 'msstats':
                ...
                if label in ["TMT", "ITRAQ"]:
                    dataset_df.loc[:, NORM_INTENSITY] = dataset_df.apply(
                        lambda x: x[field] - group_intensities[(x["Run"], x["Channel"])] +
                                  median_baseline, axis=1)
                else:
                    dataset_df.loc[:, NORM_INTENSITY] = dataset_df.apply(
                        lambda x: x[field] - group_intensities[(x["Run"], x["Fraction"])] +
                                  np.median([group_intensities[i] for i in group_intensities.keys()
                                             if i[1] == x["Fraction"]]), axis=1)
            elif nmethod == 'qnorm':
                # pivot to have one col per sample
                ref_dict = dataset_df[NORM_INTENSITY].dropna().drop_duplicates().sort_values(
                    ascending=False).reset_index(drop=True).to_dict()
                ref_dict = {v: norm_intensity[k] for k, v in ref_dict.items()}
                dataset_df.loc[:, NORM_INTENSITY] = dataset_df.apply(
                    lambda x: ref_dict[x[NORM_INTENSITY]] if x[NORM_INTENSITY] in ref_dict.keys()
                    else np.nan, axis=1)
